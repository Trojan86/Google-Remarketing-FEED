...
class VCXmlGetContentController
{
    ...
    private function processGeneration()
    {
		...
		$rss = $dom->appendChild($dom->createElement('rss')); // добавляем тег rss
 		$rss->setAttribute('xmlns:g', 'http://base.google.com/ns/1.0');//атрибуты/
		$rss->setAttribute('version', '2.0'); //атрибуты
        
        //Products
        $items = $dom->createElement("channel");
		// Shop name
        $name = $dom->createElement("title", Configuration::get('PS_SHOP_NAME'));
        $items->appendChild($name);

        // Shop url
        $url = $dom->createElement("link", $this->context->link->getPageLink('index', null, null, null, false, null, false));
        $items->appendChild($url);
		$descriptionCData = $dom->createTextNode("В этом файле перечислены товары магазина");
        ...
        foreach ($products as $product) {
			...
			$title = $dom->createElement('g:title');
			...
			//description
            $descriptionCData = $dom->createTextNode(strip_tags($product['description']));
            $description = $dom->createElement('g:description');
            $description->appendChild($descriptionCData);
            $item->appendChild($description);
			
            //url
            $url = $dom->createElement("g:link", $this->context->link->getProductLink($product));
            $item->appendChild($url);
			
			//image
            $cover = Product::getCover($product['id_product']);
            $image = $dom->createElement("g:image_link", $this->context->link->getImageLink($product['link_rewrite'], $cover['id_image']));
			...
			$price = $dom->createElement("g:price");
